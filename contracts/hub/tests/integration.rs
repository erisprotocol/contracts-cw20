...
fn config_does_not_change_exchange_rate() -> Result<()> {
    ...
    let (ustake_minted, new_token) = bond_and_harvest(&helper, router_ref)?;

    // check that the state is correct based on harvest
    let expected_response = StateResponse {
        exchange_rate: Decimal::from_ratio(new_token, ustake_minted),
        total_ustake: ustake_minted,
        total_utoken: new_token,
        unlocked_coins: vec![],
        unbonding: Uint128::zero(),
        available: Uint128::zero(),
        tvl_utoken: new_token,
    };
...
fn bond_and_harvest(
    ...
    let new_token = Uint128::new(300_000000).checked_add(bonded)?;
    Ok((ustake_minted, new_token))
}
